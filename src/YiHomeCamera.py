...
CAM_AUTH = HTTPBasicAuth(config.CAM_USER, config.CAM_PASSWORD)
...
class YiCam:
    ...
    def connectFTP(self):
        try:
            ...
            self.ftp.login(user="root", passwd=config.FTP_PASSWORD)
            self.connected = True
    ...
    def switchCamera(self, switchOn):
        ...
        try:
            ...
            x = requests.get(url, timeout=config.SETTINGS_TIMEOUT, auth=CAM_AUTH)
            if switchOn:
                y = requests.get(url, timeout=config.SETTINGS_TIMEOUT, auth=CAM_AUTH)
    ...
    def getImage(self, highQuality=False, timeStamp=True):
        ...
        response = requests.get(url, timeout=config.SNAPSHOT_TIMEOUT, auth=CAM_AUTH)
        ...
        for folder in self.ftp.nlst():
            if folder != self.tmpFile:
                ...
                for videoFile in self.ftp.nlst():
                    ...
                    urlPath = f"ftp://root:{config.FTP_PASSWORD}@{self.ip}{filePath}"
                    print(urlPath)
                    ...
                    if videoFunc:
                        videoFunc(videoObj,
                                  # name + " " +
                                  "<pre>" +
                                  folder.replace("Y", "-").replace("M", "-").replace("D", " ").replace("H", ":")
                                  + videoFile[:-4].replace("M", ":").replace("S", ".") + "</pre>",
                                  notification=notification, disable_notification=True)
